...
async function client({ options, log, modern }) {
  ...
  // The bundle's runtime publicPath is 'auto' (read from script.src), but the
  // middlewares need a static URL prefix to mount at — use the pathname
  // portion of options.publicPath (handles full URLs like a CDN gracefully).
  app.use(webpackDevMiddleware(compiler, { publicPath: options.publicPathName, stats: 'none' }))
  app.use(
    webpackHotMiddleware(compiler, {
      path: options.publicPathName + '__webpack_hmr',
      log: false
    })
  )
