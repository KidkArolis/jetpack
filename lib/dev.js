...
export default async function devServer(options, log, runtime = {}) {
...
async function client({ options, log, modern }) {
  ...
  // The bundle's runtime public path uses options.assetBaseUrl, but the
  // middlewares need a local URL prefix to mount at — use the pathname portion
  // of options.assetBaseUrl (handles full URLs like a CDN gracefully).
