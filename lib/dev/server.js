...
module.exports = async function devServer (options) {
  ...
  if (options.client) {
    log.verbose(`Client ${requireRelative.resolve(options.client, options.cwd)}`)
  }
  if (options.server) {
    log.verbose(`Server ${requireRelative.resolve(options.server, options.cwd)}`)
  }
...
async function client ({ pkg, options }) {
  ...
  if (!options.server) {
    if (options.static) {
      app.use('/static', express.static(path.join(options.cwd, options.static)))
    }
