...
import { recomputeAssets } from './options.js'
      ...
      await new Promise((resolve, reject) => {
        compiler.run(async function (err, stats) {
          try {
            ...
            options = recomputeAssets(options, stats.toJson({ entrypoints: true }))

            if (options.static && (await isDir(path.join(options.dir, options.static)))) {
              await fs.cp(path.join(options.dir, options.static), path.join(options.dir, options.dist, 'assets'), {
                recursive: true
              })
            }
            ...
            if (html) await fs.writeFile(path.join(target, modern ? 'index.html' : 'index.legacy.html'), html)
            resolve()
          } catch (err) {
            reject(err)
          }
