...
export async function options(argv = process.argv.slice(2)) {
  return (await commandOptions(argv)).config
}

export async function commandOptions(argv = process.argv.slice(2)) {
  ...
  const config = await getOptions({
    ...
    overrides: configOverrides(cli.flags)
  })

  return {
    config,
    runtime: {
      printConfig: !!cli.flags.printConfig,
      yes: !!cli.flags.yes,
      dryRun: !!cli.flags.dryRun,
      coverage: cli.flags.coverage || false
    }
  }
}
...
function configOverrides(flags) {
  return {
    ...
    target: flags.target,
    log: flags.log,
    build: flags.minify === undefined ? undefined : { minify: flags.minify }
  }
}
...
export async function run(argv = process.argv.slice(2)) {
  ...
  try {
    const { config: opts, runtime } = await commandOptions(argv)
    const log = createLogger(opts.logLevels)
    const mod = await import(`./${cli.command}.js`)
    await mod.default(opts, log, runtime)
  } catch (err) {
