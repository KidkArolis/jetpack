...
export default function serve(options) {
  ...
  const dir = path.resolve(options.root || options.dir || process.cwd())
  const dist = options.dist || 'dist'
  const host = options.host || 'localhost'
  const port = options.port || 3030
  const production = options.production ?? options.mode === 'production'

  return production ? createStaticServe(path.join(dir, dist), { dir }) : createProxy(`http://${host}:${port}`)
}
