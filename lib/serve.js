  ...
  if (!handlerPromise) {
    handlerPromise = (async () => {
      ...
      return env === 'development'
        ...
        : createServe(path.join(options.dir, options.dist), { dir: options.dir })
    })()
  }
  return handlerPromise
}

export default async function jetpackServe(req, res, next) {
...
function getModernBrowserRegex(dir) {
  ...
  if (!modernBrowserRegexes.has(key)) {
    modernBrowserRegexes.set(key, browsers.regex({ modern: true, dir: key }))
  }
