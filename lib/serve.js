      ...
      proxyReq.on('error', function (err) {
        if (err.code === 'ECONNREFUSED') {
          ...
          if (acceptHeader.includes('text/html')) {
            const errorHtmlPath = path.join(__dirname, '..', 'assets', 'error-not-running.html')
            fs.readFile(errorHtmlPath, 'utf8')
              .then((html) => {
              ...
              .catch(() => {
                res.writeHead(502, { 'Content-Type': 'text/html' })
                res.end('<h1>jetpack not running</h1><p>Failed to connect to the jetpack dev server.</p>')
              })
          } else {
