...
function readOriginalSource(source, options) {
  const candidates = [path.join(options.dir, source), path.resolve(options.dir, source)]

  for (const file of candidates) {
    if (!file.startsWith(options.dir)) continue
    try {
      return compilerReadFile(file)
    } catch {}
  }
...
function compilerReadFile(file) {
