...
function createBuildErrorItem(error, options) {
  ...
  const codeFrame = codeFrameFromBuildError(raw, file) || sourceCodeFrameFromBuildError(raw, file, options)
...
function sourceCodeFrameFromBuildError(raw, file, options) {
  ...
  const lineNumber = buildErrorLineFromRaw(raw) || findBuildErrorLine(raw, lines) || 1
