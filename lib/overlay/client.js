...
function showResolvedRuntimeError(runtimeError) {
  ...
  fetchFrameData(currentError).then(
    (data) => {
      if (currentError?.type !== 'runtime') return
  ...
  root.innerHTML = `
    ...
    <section class="jpo-dialog" role="dialog" aria-modal="true" aria-labelledby="jpo-title">
      ...
      <main class="jpo-body">
        ...
        ${renderCodeFrame(error.codeFrame || codeFrameFromPrimary(primary))}
...
function codeFrameFromPrimary(frame) {
  if (!frame?.source?.content) return null
  const lines = frame.source.content.split(/\r?\n/)
  const start = Math.max(1, frame.source.line - 3)
  const end = Math.min(lines.length, frame.source.line + 3)
  return {
    ...
    lines: lines.slice(start - 1, end).map((value, index) => ({
      number: start + index,
      value,
      highlight: start + index === frame.source.line
    }))
  }
}
