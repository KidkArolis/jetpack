...
export async function resolveOptions({
  ...
  const options = Object.assign({}, configFromFile, pick(overrides, ['port', 'hot', 'minify', 'log']))
  ...
  const target =
    overrides.legacy || overrides.modern
      ? { modern: !!overrides.modern, legacy: !!overrides.legacy }
  ...
  return clean({
    ...
    assets: {
      js: [assetBaseUrl + (target.modern ? 'bundle.js' : 'bundle.legacy.js')],
      css: [],
      runtime: [],
      other: []
    },
...
export function recomputeAssets(options, stats) {
  const { outputPath, entrypoints } = stats
  return Object.assign({}, options, {
    assets: assets({ assetBaseUrl: options.assetBaseUrl, entrypoints }),
    runtime: runtime({ outputPath, assetBaseUrl: options.assetBaseUrl, entrypoints })
  })
}
...
function assets({ assetBaseUrl, entrypoints }) {
  const js = []
  const css = []
  const other = []
  const runtime = []
  ...
  entrypoints.bundle.assets.forEach(({ name: asset }) => {
    const assetPath = assetBaseUrl + asset
...
function runtime({ outputPath, assetBaseUrl, entrypoints }) {
  let runtime = null
