...
module.exports = function options (command = 'dev', program = {}) {
  ...
  return clean({
    ...
    target: {
      modern: program.modern || (options.target && options.target.modern !== undefined ? !!options.target.modern : true),
      legacy: program.legacy || (options.target && options.target.legacy !== undefined ? !!options.target.legacy : false)
    },
...
function assets ({ root, production, modern }) {
  ...
  Object.keys(manifest).forEach(asset => {
    ...
    } else if ((asset.startsWith('bundle.') && asset.endsWith('.js')) || (asset.startsWith('vendors~') && asset.endsWith('.js'))) {
      js.push(manifest[asset])
    } else if (asset.startsWith('bundle.') && asset.endsWith('.css')) {
