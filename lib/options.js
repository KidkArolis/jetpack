...
export default async function options(command = 'dev', { entry = null, flags = {} } = {}) {
  const dir = flags.dir ? path.resolve(flags.dir) : process.cwd()
  const production = command === 'build' || command === 'inspect' || command === 'doctor'
  ...
  const options = Object.assign({}, configFromFile, pick(flags, ['port', 'dir', 'hot', 'config', 'minify', 'log']))
  ...
  return clean({
    ...
    // The URL pathname portion of `publicPath`, normalised with a trailing
    // slash. Used by the dev server to mount middlewares at the right
    // location. Works whether publicPath is a path (`/assets/`) or a full
    // URL (`https://cdn.example.com/bundles/`).
    publicPathName: pathnameWithTrailingSlash(publicPath),
