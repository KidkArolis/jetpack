...
const REMOVED_CONFIG_OPTIONS = {
  static: null,
  proxy: null,
  exec: null,
  react: null,
  head: 'Use html.render instead.',
  body: 'Use html.render instead.',
  webpack: 'Use rspack instead.'
}
const REMOVED_CONFIG_FILE_OPTIONS = {
...
export async function resolveOptions({
  ...
  const configPath = resolveConfigPath(dir, configFile)

  const configFromFile = await readConfigFromFile(configPath)
  validateConfigFileOptions(configFromFile)
  validateOverrideOptions(overrides)
  ...
  const build = normalizeBuild(options.build, { mode })
...
function resolveConfigPath(dir, configFilePath) {
  ...
  if (configFilePath) {
    const configPath = path.resolve(dir, configFilePath)
...
function validateConfigFileOptions(config) {
  validateMovedConfigOptions(config)
  validateRemovedConfigOptions(config, REMOVED_CONFIG_FILE_OPTIONS)
}

function validateOverrideOptions(config) {
  validateMovedConfigOptions(config)
  validateRemovedConfigOptions(config)

  for (const name of Object.keys(config)) {
    if (!CONFIG_OPTION_KEYS.includes(name)) {
...
function validateMovedConfigOptions(config) {
  for (const [from, to] of Object.entries(MOVED_CONFIG_OPTIONS)) {
...
function validateRemovedConfigOptions(config, removedOptions = REMOVED_CONFIG_OPTIONS) {
  for (const [name, guidance] of Object.entries(removedOptions)) {
    if (config[name] !== undefined) {
      throw new Error(`${name} is no longer supported.${guidance ? ` ${guidance}` : ''}`)
    }
  }

  if (config.css?.features !== undefined) {
    throw new Error('css.features is no longer supported. Use the rspack hook instead.')
  }
}

function normalizeBuild(input, { mode }) {
...
function parseLogLevels(input = 'info,progress') {
