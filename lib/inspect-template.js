...
const d3HierarchyPkg = findPackageRoot(d3HierarchyEntry, 'd3-hierarchy')
...
function findPackageRoot(entry, packageName) {
  ...
  while (path.basename(current) !== packageName) {
    const parent = path.dirname(current)
    if (parent === current) {
      throw new Error(`Could not find the ${packageName} package root from ${entry}.`)
    }
    current = parent
  }
